...
export default function Dashboard() {
  ...
  const stats = useQuery(api.stats.stats.getDashboardStats, { dateRange, jobFilter });
  ...
  return (
    <div className="self-stretch bg-background pb-[133px] min-h-screen w-full">
      ...
      <div className="flex items-start self-stretch mb-[19px] ml-[21px] mr-[43px]">
        ...
        <button
          className="flex shrink-0 items-center bg-surface text-text-primary text-left py-2 px-4 mt-[18px] mr-2.5 gap-2 rounded-md border border-solid border-border hover:bg-surface-container-high transition-colors cursor-pointer"
          onClick={() => router.push('/dashboard/ingestion-monitor')}
        >
          <Upload className="w-4 h-4 text-emerald-900 dark:text-emerald-400" />
        ...
        >
          ...
          <span className="text-on-primary text-[13px] font-bold">Create Job</span>
        </button>
      </div>
